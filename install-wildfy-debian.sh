...
sudo cp /opt/wildfly-9.0.1.Final/bin/init.d/wildfly.conf /etc/default/wildfly
# Changes to configure:
sudo sed -i 's|# JAVA_HOME="/usr/lib/jvm/default-java"|JAVA_HOME="/usr/lib/jvm/java-8-oracle"|g' /etc/default/wildfly
sudo sed -i 's|# JBOSS_HOME="/opt/wildfly"|JBOSS_HOME="/opt/wildfly-9.0.1.Final"|g' /etc/default/wildfly
...
# Changes in Init
sudo sed -i 's|# Provides:             wildfly|# Provides:             wildfly|g' /etc/init.d/wildfly
sudo sed -i 's|NAME=wildfly|NAME=wildfly|g' /etc/init.d/wildfly
