...
create_snapshot() {
    ...
    VOL_ID=`${AWS} ec2 describe-volumes  --output text | grep ${INSTANCE_ID} | awk '{print $7}'`
    if [ -z ${VOL_ID} ] ; then
        echo ${VOL_ID}
    ...
    ${AWS} ec2 create-snapshot --volume-id ${VOL_ID} --description "Daily Backup(${INSTANCE_ID}) from ${VOL_ID}" >> ${LOG_FILE} 2>&1
    if [ $? != 0 ] ; then
...
delete_old_snapshot() {
    ...
    SNAPSHOTS=`${AWS} ec2 describe-snapshots --output text | grep ${VOL_ID} | grep "Daily Backup" | wc -l`
    while [ ${SNAPSHOTS} -gt ${SNAPSHOTS_PERIOD} ]
    do
        ${AWS} ec2 delete-snapshot --snapshot-id `${AWS} ec2 describe-snapshots --output text | grep ${VOL_ID} | grep "Daily Backup" | sort -k 11,11 | awk 'NR==1 {print $8;exit}'` >> ${LOG_FILE} 2>&1
        if [ $? != 0 ] ; then
            print_msg "ERR:${SHELLDIR}/${SHELLNAME} ec2-delete-snapshot"
            logger -f ${LOG_FILE}
            exit 1
        fi
        SNAPSHOTS=`${AWS} ec2 describe-snapshots | grep ${VOL_ID} | grep "Daily Backup" | wc -l`
    done
